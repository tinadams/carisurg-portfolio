...
| AI Output Is Hard for Clinical Staff to Understand           | AI-Technical |          M |      H | The AI will give a short, clear reason for each alert. e.g “high risk because of low blood pressure 70/40 mmHg and high heart rate 145 bpm.”                    | Nurses can clearly understand and communicate the reason for each AI alert.                           |
| Alert Fatigue (Too Many Alerts)                                 | Operational  |          H |      H | Only send alerts for serious cases that need attention. Allow nurses to set alert preferences, such as audio, visual, or colour-coded alerts.                                              | Nurses set alert preferences and do not experience alert fatigue during the testing period.  |
| Workflow Disruption (Triage Takes Longer)                          | Operational  |          M |      H | The AI flag appears automatically, without requiring extra steps from nurses.                                    | Average triage time does not increase after the tool is introduced.                                     |
...
| Over-Reliance on AI Output (Nurses Trust the AI Too Much)                    | Ethical      |          M |      H | Train staff that the AI is decision support only. Nurses must remain responsible for final triage decisions and can override the AI.                                        | Nurses still use their own judgment and override the AI when needed. Override rates are tracked, since a sudden drop may suggest over-reliance. |
| No Clear Person Responsible                  | Ethical      |          M |      H | Decide and define who is responsible for model approval, monitoring, clinical use, and response to harm before deployment.                                                             |  A clinical lead and technical lead are clearly named with documented responsibilities and everyone knows who to contact if something goes wrong.                  |
...
### 1. Poor Local Model Performance in Caribbean ED
...
Alert fatigue is a serious operational risk in busy EDs. If the system sends too many warnings, triage nurses may become overwhelmed and find it harder to respond quickly and consistently to each patient who needs attention. Even when alerts are clinically meaningful, too many at once could reduce patient safety rather than improve it.

The mitigation is to keep alerts limited, clear, and linked to a specific next step. The tool should only flag cases where early attention could realistically improve the patient’s care. The signal of success is that alerts are followed by appropriate next steps, such as reassessment, escalation, or earlier communication with the ED team.
